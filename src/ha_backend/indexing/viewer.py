...
def find_record_for_snapshot_fields(
    *,
    warc_path: str | PathLike[str],
    warc_record_id: str | None,
    url: str | None,
) -> Optional[ArchiveRecord]:
    """
    Locate the WARC response record corresponding to copied Snapshot fields.
    ...
    if warc_record_id:
        record = _load_matching_html_record(path, target_record_id=warc_record_id)
        if record is not None:
            return record

    # Fallback: first record matching URL.
    return _load_matching_html_record(path, target_url=url)
...
def find_record_for_snapshot(snapshot: Snapshot) -> Optional[ArchiveRecord]:
    ...
    """
    return find_record_for_snapshot_fields(
        warc_path=snapshot.warc_path,
        warc_record_id=snapshot.warc_record_id,
        url=snapshot.url,
    )


__all__ = ["find_record_for_snapshot", "find_record_for_snapshot_fields"]
