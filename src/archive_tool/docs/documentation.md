...
* `--browsertrix-config-json`
  ...
  * If set, the tool persists the supplied JSON object as a stable Browsertrix
    config file under `--output-dir` and passes it to zimit via `--config` for
    fresh/new crawl phases.
  * This exists for Browsertrix-only settings that the deployed zimit image may
    reject on its mixed zimit/warc2zim CLI surface (for example
    `extraChromeArgs`).
  * Resume phases still use the discovered/persisted resume YAML generated by
    the crawler itself.
