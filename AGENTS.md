# AGENTS.md – HealthArchive.ca monorepo

## Project overview

- Backend for HealthArchive.ca:
  - Repo shape: backend at the repo root, frontend in `frontend/`, datasets separate.
...
Canonical public docs to consult first:

- **Documentation Site**: Run `make docs-serve` locally to view the docs hub for this repo.
- **LLM Context File**: public-safe context generated by
  `scripts/generate_llms_txt.py` (written to docs/llms.txt; regenerated by
...
- `docs/planning/roadmap.md` - backlog of not-yet-implemented public work.
- `docs/planning/implemented/` - historical implementation plans.
...
Do not add, infer, or preserve private operational details in public files, including:

- production hostnames
- private network names
- private IP addresses
- exact VPS paths
- host-side filesystem paths
- release roots
- env-file paths
- deployment inventories
- monitoring/alerting internals
- backup locations
- operator-only runbooks
- personal application planning unrelated to this project

Public documentation may include project purpose, methodology, architecture
overview, limitations, public data handling, and reproducible local development
instructions.

Production deployment details, shared VPS inventory, private host
configuration, monitoring configuration, and operator-only runbooks are
...
- Run backend-only CI parity explicitly:

  ```bash
  make backend-ci
  ```

- Install frontend dependencies and run frontend parity:
...
- Run integrated backend + frontend smoke from one checkout:

  ```bash
  make integration-e2e
  ```

- Run the local pre-push parity gate (recommended before any push):
...
- Frontend toolchain note:
  ...
  ```bash
  node --version  # frontend requires >= 20.19.0
  ```

- **Documentation**:

  ```bash
  make docs-serve  # View the current docs site locally
  make docs-build  # Build the current static docs site to site/
  ```

- Docs-platform note:

  - The current docs portal is still MkDocs-based.
  - Until the migration lands, treat `mkdocs.yml` as the live nav source of
    truth.
  - Keep policy/content changes generator-agnostic unless the task is
    explicitly about docs-platform migration work.
...
- Run the closest relevant local checks before pushing (use `make prepush` by default; use `make check-full` before deploys).
...
- Commits, docs, code comments, and release notes must list only human contributors as authors, except that `dependabot[bot]` and `github-actions[bot]` authorship is permitted for their own automation commits.
- Do not rely on bot-managed auto-merge paths that stamp broad bot authorship or unnecessary co-author trailers into repository history.
- If a bot proposes a change (for example Dependabot), prefer landing any accepted update in a new human-authored commit; direct bot-authored dependency commits are acceptable only for the permitted bot identities above.
...
- **Update Navigation**: When adding new files, update the current docs-site nav
  config (today: `mkdocs.yml`) if you want them in the sidebar.
