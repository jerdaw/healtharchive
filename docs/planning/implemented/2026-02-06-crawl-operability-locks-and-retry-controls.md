...
- Job locks no longer force `/tmp`-style `1777` semantics on dedicated lock directories; production now uses `HEALTHARCHIVE_JOB_LOCK_DIR=<service-data-root>/ops/locks/jobs`.
...
- Completed the production lock-dir cutover on 2026-04-14 by restarting the API and worker with `/etc/healtharchive/backend.env` already pointing at `<service-data-root>/ops/locks/jobs`.
