...
- Reuse the existing `healtharchive/scripts/verify_public_surface.py`
...
- Add GitHub Actions jobs to run the smoke check in:
  - `healtharchive` CI (checks backend changes against latest frontend `main`),
...
4. Update docs:
   ...
   - Remove this item from `docs/planning/roadmap.md`.

## Acceptance criteria

- Running the smoke locally succeeds:
  - `healtharchive/scripts/ci-e2e-smoke.sh`
