...
3. Recover stale running jobs (safe dry-run first):
   - `<deploy-root>/.venv/bin/healtharchive recover-stale-jobs --older-than-minutes 180`
   - Apply (sets `status=retryable`): `<deploy-root>/.venv/bin/healtharchive recover-stale-jobs --older-than-minutes 180 --apply`
