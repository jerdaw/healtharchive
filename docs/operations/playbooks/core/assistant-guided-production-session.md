...
- Do not edit `<deploy-root>` directly during the session.
...
- HealthArchive inspection:
  - `<deploy-root>/.venv/bin/healtharchive list-jobs --limit 50`
  - `<deploy-root>/.venv/bin/healtharchive show-job --id <JOB_ID>`
  - `cd <deploy-root> && ./scripts/vps-crawl-status.sh --year <YEAR>`
- bounded diagnostics:
...
- `<deploy-root>/.venv/bin/healtharchive recover-stale-jobs --apply ...`
- `<deploy-root>/.venv/bin/healtharchive retry-job ...`
- `<deploy-root>/.venv/bin/healtharchive patch-job-config ...`
- `<deploy-root>/.venv/bin/healtharchive cleanup-job ...`
...
- hotfix the backend by editing files under `<deploy-root>`
...
Cmnd_Alias HA_SESSION_COMMANDS = \
    ...
    /usr/bin/journalctl, \
    /usr/bin/docker, \
    <deploy-root>/.venv/bin/healtharchive, \
    <deploy-root>/scripts/vps-deploy.sh

Defaults:haadmin use_pty, env_reset
Defaults:haadmin iolog_dir="<sudo-io-log-dir>/%{user}"
