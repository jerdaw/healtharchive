...
- If the VPS reboots (or the worker/service is killed) mid-crawl, a job can be left in `status=running`. Recover safely:
  ...
  - Dry-run: `<deploy-root>/.venv/bin/healtharchive recover-stale-jobs --older-than-minutes 180`
  - Apply: `<deploy-root>/.venv/bin/healtharchive recover-stale-jobs --older-than-minutes 180 --apply`
