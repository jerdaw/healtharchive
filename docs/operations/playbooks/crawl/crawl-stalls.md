...
<deploy-root>/.venv/bin/healtharchive list-jobs --status running --limit 10
```
...
```bash
JOBDIR="<service-data-root>/jobs/SOURCE/YYYYMMDDTHHMMSSZ__name"
...
- If the watchdog is enabled but prints `SKIP ... max recoveries reached`, you can still do the manual recovery above, or (carefully) run the watchdog script once with a higher cap:
  ```bash
  sudo bash -lc 'set -a; source /etc/healtharchive/backend.env; set +a; <deploy-root>/.venv/bin/python3 <deploy-root>/scripts/vps-crawl-auto-recover.py --apply --max-recoveries-per-job-per-day 4'
  ```
