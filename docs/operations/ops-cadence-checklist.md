...
- **Verify observability is still healthy** (internal; loopback-only):
  - `cd <deploy-root> && ./scripts/vps-verify-observability.sh`
...
- **Disk usage trend**
  - `df -h /`
  - If `<service-data-root>` exists: `du -sh <service-data-root>/* | sort -h | tail -n 5`
...
- Keep systemd unit templates installed/updated on the VPS after repo updates:
  - `cd <deploy-root> && sudo ./scripts/vps-install-systemd-units.sh --apply --restart-worker`
...
- If you use Healthchecks pings, periodically audit for drift (missing or stale checks):
  - `cd <deploy-root> && sudo python3 ./scripts/verify_healthchecks_alignment.py`
...
- **Adoption signals entry** (public-safe)
  - Add a dated entry under `<service-data-root>/ops/adoption/` (links + aggregates only).
...
- **Automation posture check**
  - On the VPS run: `cd <deploy-root> && ./scripts/verify_ops_automation.sh`
  - Optional (diff-friendly): `./scripts/verify_ops_automation.sh --json | python3 -m json.tool`
  - Optional (JSON-only artifact): `./scripts/verify_ops_automation.sh --json-only > <service-data-root>/ops/automation/posture.json`
...
- **Annual edition readiness**
  ...
  - Run the crawl preflight audit:
    - `cd <deploy-root> && YEAR=2026 && ./scripts/vps-preflight-crawl.sh --year "$YEAR"`
