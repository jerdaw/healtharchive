...
When drift is limited to a file generated by one of the VPS installer helpers,
prefer re-running the matching installer instead of editing the baseline policy
or patching the file by hand.

Example:

- If drift reports `/etc/grafana/provisioning/dashboards/healtharchive.yaml`,
