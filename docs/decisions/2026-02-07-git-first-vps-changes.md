...
- The production VPS uses a git checkout at `<deploy-root>` as the deploy source of truth.
...
- Operator friction was observed when ad-hoc `scp`-copied scripts (or local edits) landed in `<deploy-root>`, causing:
...
- We will treat `<deploy-root>` as a **git-managed** deploy artifact only:
...
- Continue allowing ad-hoc `scp` into `<deploy-root>` — rejected: leads to dirty-tree deploy failures and untracked production drift.
...
- Verification:
  - `cd <deploy-root> && git status --porcelain` is empty before deploys.
